...
namespace ConciliadoraTestes.Testes.Vendas.ConferenciaDeVendas
{
    public class MetodosConferenciaDeVendas
    {
        ...
        public void AbrirConferenciaDeVendas()
        {
            ...
            IWebElement dashboard = inicializaDriver.ObterDriver().FindElement(By.XPath("//i[@class='kt-menu__link-icon menuSideIcon fas fa-search-dollar']")); // Localiza o ícone de pagamentos e clica
            dashboard.Click();

            inicializaDriver.ObterDriver().Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(30);

            IWebElement gerencial = inicializaDriver.ObterDriver().FindElement(By.XPath("//span[text()='Conferência de Vendas']")); //Localiza pelo texto exibido
