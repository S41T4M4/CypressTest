...
{
    public class MetodosConciliacaoBancaria
    {
        protected InicializaDriver inicializaDriver = new InicializaDriver();
        private EncerraDriver encerra = new EncerraDriver();

        public void AbrirConciliacaoBancaria()
        {
            ...
            IWebElement administrativo = inicializaDriver.ObterDriver().FindElement(By.XPath("//i[@class='kt-menu__link-icon menuSideIcon fas fa-university']"));
            administrativo.Click();

            IWebElement conciliacaoBancaria = inicializaDriver.ObterDriver().FindElement(By.XPath("//span[text()='Conciliação Bancária']"));
            inicializaDriver.ObterDriver().Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(30);
            conciliacaoBancaria.Click();
        }
